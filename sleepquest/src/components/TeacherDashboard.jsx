...
export default function TeacherDashboard() {
  const { teacherState, isLoading, classData, questions, logout, refreshData } = useTeacher();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-slate-100 dark:from-slate-900 dark:via-indigo-950 dark:to-slate-900 transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">📝 ניהול שאלות</h2>
          <AddQuestionForm />
        </div>
...
function TeacherHeader({ teacherName, className, onLogout, onRefresh }) {
  return (
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white">SleepQuest Teachers</h1>
...
function SubmissionsSection({ classData }) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">📊 הגשות תלמידים</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700/50">
              {classData.submissions && classData.submissions.map((submission, idx) => {
                const percentage = Math.round((submission.submitted / submission.total) * 100);
                ...
                const timeStr = submission.submittedAt
                  ? new Date(submission.submittedAt).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })
                  : '-';
...
function QuestionsStatusSection({ questions }) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">📋 סטטוס שאלות</h2>
          ...
          <div className="space-y-3">
            {approvedQuestions.length > 0 ? (
              approvedQuestions.map((q) => (
                <div key={q.id} className="p-3 bg-green-50 dark:bg-green-500/10 rounded-lg border border-green-200 dark:border-green-500/30">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">אין שאלות מאושרות</p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {pendingQuestions.length > 0 ? (
              pendingQuestions.map((q) => (
                <div key={q.id} className="p-3 bg-yellow-50 dark:bg-yellow-500/10 rounded-lg border border-yellow-200 dark:border-yellow-500/30">
                  <p className="text-sm text-slate-900 dark:text-white">{q.text}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    סוג: {getQuestionTypeLabel(q.type)}
                  </p>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">אין שאלות בהמתנה</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-lg p-4">
        <p className="text-sm text-blue-600 dark:text-blue-300 flex items-start gap-2">
          ...
          <span>
            רק שאלות מאושרות מופיעות בשאלון של התלמידים.
            שאלות בהמתנה יופיעו לאחר אישור המנהל.
          </span>
        </p>
      </div>
    </div>
