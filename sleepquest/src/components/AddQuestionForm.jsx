...
/**
 * AddQuestionForm - Checkbox Fixed
 * Now properly handles checkbox (yes/no) questions
 */
export default function AddQuestionForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // For radio questions, validate options
    ...
    // For checkbox (yes/no), automatically set options to ["כן", "לא"]
    const questionOptions = questionType === 'checkbox'
      ? ['כן', 'לא']
      : questionType === 'radio'
      ? options.filter(opt => opt.trim())
      : [];

    const questionOptionsEmoji = questionType === 'checkbox'
      ? ['✅', '❌']
      : questionType === 'radio'
    ...
    <div className="bg-white/80 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden transition-colors duration-300">
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-700/50 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-slate-800/80 dark:to-slate-800/60">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">הוסף שאלה חדשה</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                {isFormDisabled ? '❌ הבעת למקסימום של 5 שאלות' : `${questionCount}/${MAX_QUESTIONS} שאלות בשימוש`}
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 h-2 bg-slate-200 dark:bg-slate-700/50 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all duration-500 ${
              questionCount >= MAX_QUESTIONS ? 'bg-red-500' : questionCount >= 3 ? 'bg-yellow-500' : 'bg-green-500'
            }`}
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div>
          ...
          />
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{questionText.length} / 200 תווים</p>
        </div>
        ...
        {questionType === 'checkbox' && (
          <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-blue-600 dark:text-blue-300">
              ℹ️ שאלות כן/לא יציגו אוטומטית שתי אפשרויות: <strong>כן</strong> ו<strong>לא</strong> עם אימוג'ים ✅ ❌
            </p>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-3">
              {options.map((option, index) => (
                <div key={index} className="flex gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handleOptionEmojiChange(index, e.target.value.slice(0, 2))}
                    disabled={isFormDisabled}
                    maxLength={2}
                    placeholder="😀"
              ...
              <button
                type="button"
                onClick={addOption}
                disabled={isFormDisabled || options.length >= MAX_OPTIONS_PER_QUESTION}
                className={`w-full px-4 py-2 border-2 border-dashed border-green-500 dark:border-green-400 text-green-600 dark:text-green-400 rounded-lg font-medium transition-colors ${
                  isFormDisabled || options.length >= MAX_OPTIONS_PER_QUESTION ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-50 dark:hover:bg-green-500/10'
                }`}
              >
                + הוסף אפשרות
              </button>
        ...
        <div className="p-4 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-xl">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-3 cursor-pointer rounded-lg bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">
              ...
              />
              <div>
                <div className="font-medium text-slate-900 dark:text-white">הכיתה שלי בלבד</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">השאלה תופיע רק לתלמידי הכיתה שלי</div>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 p-3 cursor-pointer rounded-lg bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">
              ...
              />
              <div>
                <div className="font-medium text-slate-900 dark:text-white">כל הכיתות</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">השאלה תופיע לכל התלמידים בבית הספר</div>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 p-3 cursor-pointer rounded-lg bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">
              ...
              />
              <div>
                <div className="font-medium text-slate-900 dark:text-white">כיתות ספציפיות</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">בחר את הכיתות שיראו את השאלה</div>
              </div>
            </label>
          </div>
          ...
          {classSelection === 'specific' && (
            <div className="mt-4 pt-4 border-t border-blue-200 dark:border-blue-500/30">
              <div className="grid grid-cols-2 gap-2">
                {allClasses.map((cls) => (
                  <label key={cls.id} className="flex items-center gap-2 p-2 cursor-pointer rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 text-green-600"
                    />
                    <span className="text-sm text-slate-700 dark:text-slate-300">{cls.name}</span>
                  </label>
                ))}
              </div>
            </div>
